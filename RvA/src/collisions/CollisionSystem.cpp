	...
	if (m_enableDebugView) {
		for (Collision& collision : m_collisions) {
			if (collision.event == CollisionEvent::Enter) {
				...
				m_debugDrawingList.addTimedAction(
					[bbA, bbB]() {
...
void CollisionSystem::manageScheduledDestruction() {
	for (auto& handle : m_scheduledDestruction) {
		...
		for (auto& otherColliderHandle : collider->collidingWith) {
			...
			m_collisions.push_back({otherCollider->flag | collider->flag, CollisionEvent::Exit, collider, otherCollider});
		}
...
void CollisionSystem::manageCollisions() {
	...
	m_colliders.forEachActive([this, &i](ColliderHandle handleA, Collider& cA) {
		m_colliders.forEachActive(
			[this, &i, &cA](ColliderHandle handleB, Collider& cB) {
				...
				if (shouldCheck) {
					if (CheckCollisionRecs(cA.boundingBox, cB.boundingBox)) {
						auto firstCollision = cA.collidingWith.find(cB.handle) == cA.collidingWith.end() && cB.collidingWith.find(cA.handle) == cB.collidingWith.end();
						...
						auto wasColliding = cA.collidingWith.find(cB.handle) != cA.collidingWith.end() || cB.collidingWith.find(cA.handle) != cB.collidingWith.end();
						if (wasColliding) {
...
void CollisionSystem::draw() {
	if (m_enableDebugView) {
		m_colliders.forEachActive([](auto handle, auto& collider) { DrawRectangleLinesEx(collider.boundingBox, 1, YELLOW); });

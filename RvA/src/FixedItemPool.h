...
template<typename T, size_t Capacity> class FixedItemPool {
public:
	FixedItemPool() {
	...
	void releaseItem(int index) {
		if (index < 0 || index >= static_cast<int>(Capacity) || !used[index]) {
			return;
		}
	...
	template<typename Func> void forEachActive(Func&& fn, int startFrom = 0) {

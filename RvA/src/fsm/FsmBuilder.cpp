...
const HashId FsmBuilder::appExitRequestTransition = "_appExitRequest";
...
FsmBuilder& FsmBuilder::state(std::string_view stateName, std::unique_ptr<FsmState> state) {
	...
	if (_states.contains(hashedStateName)) {
		assert(0 && "Can't add state. State already exists");
	}
	_latestOnTransition = "";
...
FsmBuilder& FsmBuilder::_on(const HashId& transitionName) {
	_latestOnTransition = "";
	if (!_latestState) {
		assert(0 && "Can't create a transition. No state has been setup in the FsmBuilder");
	}

	if (_latestState->transitions.contains(transitionName)) {
		assert(0 && "Can't create a transition. Can't create a transition with the same name");
	}
...
FsmBuilder& FsmBuilder::_jumpTo(std::function<bool()> condition, const HashId& targetState) {
	if (!_latestState) {
		assert(0 && "Can't add a target state to jumpTo. No state has been setup in the FsmBuilder.");
	}

	if (_latestOnTransition.empty()) {
		assert(0 && "Can't add a target state to jumpTo. No 'on' event has been defined yet. State [{}], TargetState [{}]");
	}
...
std::tuple<std::unique_ptr<Fsm>, std::unique_ptr<FsmInfo>> FsmBuilder::build(const HashId& startStateName, FsmExternalController* externalController) {
	if (!_states.contains(startStateName)) {
		...
		assert(0 && "Can't build FSM. No states have been added to the FSM");
	}
